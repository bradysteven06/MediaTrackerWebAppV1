...
function getFilteredEntries() {
  ...
  let filtered = mediaList.filter((entry) => {
    ...
    const matchesGenre =
      !genreFilter ||
      (Array.isArray(entry.genres) && entry.genres.includes(genreFilter));
...
function formatEntryHTML(entry) {
  ...
  return `
        <div class="entry-row">
            ...
            <div class="entry-sub">
                Genres: ${genresHTML} &nbsp;-&nbsp; Rating: ${escapeHTML(
    rating
  )}
            </div>
            ${
              notes
                ? `<div class="entry-notes"><small>${notes}</small></div>`
                : ""
            }
...
function renderEntries() {
  ...
  const html = filtered
    .map((entry) => `<li>${formatEntryHTML(entry)}</li>`)
    .join("");
...
function deleteEntryById(id) {
  ...
  const name = mediaList[idx]?.title
    ? `"${mediaList[idx].title}"`
    : "this entry";
